...
const Technologies: React.FC<ITechnologies> = ({
  field,
  technologies,
  color,
}) => {
...
const ProjectDetail: React.FC<IProjectDetail> = (props) => {
  const { title, about, image, source, technologies, website, year, color } =
    props.project || {};
  ...
  const moreInfoOpenHandler = () => {
    ...
    props.setMoreInfoOpen(true);
  };
  return (
    //
    <AnimatePresence>
      {props.project && props.detailOpen && !props.moreInfoOpen ? (
        <motion.div
